...
function*Fi() {
  ...
  for (;;) {
    ...
    if (alpha) {
      ...
      // Preserve untouched bands while the next sample is accumulated.
    ...
    for (let row = 0; row < rows; row++) for (let column = 0; column < columns; column++) {
      ...
      y = rows - y - 1;
      primary.scissor.set(
        offsetX + Math.floor(x * pixelWidth / columns), offsetY + Math.floor(y * pixelHeight / rows),
        Math.floor((x + 1) * pixelWidth / columns) - Math.floor(x * pixelWidth / columns),
        Math.floor((y + 1) * pixelHeight / rows) - Math.floor(y * pixelHeight / rows),
      );
