...
export function createPalm({
  ...
  if (values.some(value => !Number.isFinite(value)) || height <= 0 || thickness <= 0 || length <= 0 || width <= 0 || spread <= 0 ||
      ...
      !Number.isInteger(coconuts) || coconuts < 0 || coconuts > 8 || !['blade', 'feather', 'fan'].includes(style)) {
    throw RangeError('Invalid palm shape');
  }
  const root = new THREE.Group();
  root.name = 'PalmVariation';
  const bark = new THREE.MeshStandardMaterial({ color: trunk, roughness: 0.85, flatShading: true });
  const band = bark.clone();
  ...
  for (let index = 0; index < rings; index++) {
    ...
    const segment = new THREE.Mesh(new THREE.CylinderGeometry(radius * 0.95, radius, direction.length() * 1.06, 7), bark);
    segment.position.copy(start).add(end).multiplyScalar(0.5);
    segment.quaternion.setFromUnitVectors(axis, direction.clone().normalize());
    root.add(segment);
    const ring = new THREE.Mesh(new THREE.CylinderGeometry(radius * 1.02, radius * 1.08, height / rings * 0.12, 7), band);
